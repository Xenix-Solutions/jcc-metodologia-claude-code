...
- **Cada fase es una skill.** Los prompts de fase viven en **skills globales** que se invocan como slash commands (`/jcc-design`, `/jcc-spec`, `/jcc-implement`, `/jcc-review`, `/jcc-handoff`). Un toque en vez de un copy-paste. La superficie completa: **4 fases + cierre + 2 herramientas auxiliares fuera del ciclo** (`/jcc-consulta`, `/jcc-auditoria`).
...
- **Perfil vigente (2026-07-30): Opus 4.8 con effort `xhigh` en todas las fases.** Decisión operativa de estabilización tras la primera semana con Opus 5 (adherencia excesiva, verbosidad, equivocaciones nuevas — ver el análisis externo del 30-07 en `evolucion-metodologia/`): se vuelve al perfil que llevaba semanas funcionando bien mientras el ecosistema Opus 5 madura. Es el **primer uso real de la capa de calibración en sentido inverso**: solo se conmutaron las líneas etiquetadas; todo lo durable quedó intacto. **Nada impide usar otro modelo** — Fable 5 si necesitas el techo de capacidad; y para ejecutar un contrato ya cerrado al pie de la letra (un spec sin decisiones pendientes), la adherencia de Opus 5 en `high` es ventaja, no defecto: **cada sesgo en su sitio**.
- **Pauta para el regreso a Opus 5** (validada en v1.2.2–v1.3.1; espera aquí, no caduca): `high` como estándar de sesión —a effort alto los turnos conversacionales deliberan de más y el modelo se percibe torpe—; la profundidad extra para lanzar una implementación grande la pone **`ultrathink`** en el turno de aprobación del plan (no cambia el effort enviado ni invalida la caché); **`xhigh`** solo al abrir sesión fresca dedicada a implementar; la Fase 4 rinde a effort bajo; la verbosidad se mitiga con instrucción explícita de concisión (la mitigación oficial: bajar el effort no acorta la respuesta visible). Al volver: re-conmutar las etiquetas de calibración y revisar la vigencia de la inyección de contención de delegación del system prompt de Claude Code (solo-Opus-5, "heron_brook"), que con 4.8 no aplica.
...
Tres reglas anejas (v1.3 y v1.4, de la evidencia de las adopciones):

- **Mapas vivos vs fotos** *(v1.4)*. Los índices y punteros (README, índice global, "Fase actual", Backlog) son **mapas vivos**: se corrigen cuando la realidad cambia. Los documentos fechados (HANDOFF, REVIEW, BRIEF, AUDITORIA, INVESTIGACION) son **fotos**: no se reeditan — lo que los supera se registra en un documento nuevo o en un ADDENDUM fechado.
...
- **Fase actual:** <puntero CORTO al trabajo ACTIVO>. Contiene solo: cambio/ciclo activo · fase ·
  ...
  `docs/cambios/README.md` (los que existan). Si no hay trabajo activo: "ninguno; el siguiente arranca con `/jcc-design`
...
- **Arranque: reconstruye antes de opinar.** Si tu tarea toca el trabajo en curso — aunque sea
  ...
  estado: índice global `docs/cambios/README.md` → README del cambio activo → su último
  handoff, y contrasta con `git log`. "Leo lo que necesite" no funciona: para saber qué
  necesitas, primero mira el mapa. Y OJO: reconstruir contexto NO es entrar en las fases —
  una sesión que SOLO consulta o comprueba no avanza fases, no toca "Fase actual" y no ofrece
  `/jcc-handoff` al terminar (si acaba tocando código o documentos, ya no es solo-consulta:
  aplica el contrato de abajo).
- **Operas como COPILOTO.** En las transiciones de fase, recuerda y ofrece el command que toca
  ...
  **Ajustas la longitud de cada documento a lo que el trabajo pide**: recorta relleno, no conectivas
  ni contexto — el criterio es que un humano pueda releerlo, no la brevedad.
...
- `BRIEF_<slug>.md` — material de partida (de negocio, de un tercero, para una herramienta de diseño), o vista materializada por `/jcc-consulta` (fechada; foto que no se mantiene).
- `INVESTIGACION_<slug>.md` — investigación/spike que alimenta una decisión.
- `AUDITORIA_<slug>.md` — informe de `/jcc-auditoria` (fechado; hallazgos sobre la propia documentación, solo reporta).
...
Cada fase **describe** su objetivo y deja un artefacto; el **prompt operativo vive en su skill** (global, `~/.claude/skills/jcc-<nombre>/SKILL.md`, invocada como slash command). El estado y los índices son locales al proyecto.

> **Respaldo de las skills.** La copia versionada vive en [`skills/`](../skills/) de este repo y es la **fuente de verdad**. En una máquina nueva, instálalas con `skills\install.ps1`; `skills\install.ps1 -Check` te dice si la copia viva y la del repo han divergido. Todas llevan `disable-model-invocation: true`: las invocas tú, nunca se autodisparan.
...
Fuera del ciclo de fases hay **dos herramientas auxiliares**, también skills: `/jcc-consulta` (leer la documentación como historia; ver *Vistas para humanos* en Transversal) y `/jcc-auditoria` (auditarla; ver su entrada en Transversal). Ninguna avanza fases ni toca los hogares.
...
- **Vistas para humanos (`/jcc-consulta`; no es una fase).** La documentación JCC está optimizada para retomar el trabajo; la lectura fácil —ponerse al día, la historia de un cambio, el estado para una reunión, un brief para cliente— se **pide, no se mantiene**: un documento-resumen mantenido sería un cuarto hogar duplicando verdad. `/jcc-consulta` genera la vista bajo demanda al nivel, tono y audiencia que toque (sin argumentos: orientación — el mapa del proyecto y "¿sobre qué hablamos?"). Si la vista debe viajar, se **materializa** como `BRIEF_AAAAMMDD_<slug>.md` registrado en el README: una **foto** que no se mantiene; si hace falta otra, se regenera desde la verdad.
- **Auditoría de la documentación (`/jcc-auditoria`; tampoco es una fase).** Revisión independiente y adversarial de los propios documentos ("asume que la documentación miente"): capas **mecánica** y **semántica** siempre; **contraste con la realidad** (¿lo que la documentación afirma del código/BD/infraestructura es verdad HOY?) solo **opt-in** y con alcance acotado, por coste. Solo lectura: entrega `AUDITORIA_AAAAMMDD_<slug>.md` (hallazgos con `fichero:línea`, todo reportado con nivel de confianza) y **no corrige** — el operador decide, y la corrección posterior aplica el backport completo. Momentos típicos: cierre de ciclo o cambio, síntoma de documentación mentirosa, antes de un onboarding; la capa de realidad, antes de un traspaso o un corte.
