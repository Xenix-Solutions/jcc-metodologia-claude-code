...
| **Puerta de entrada** — todos los cambios del proyecto | **`docs/cambios/README.md`** (índice global) | tabla estable de navegación; una fila por cambio |
...
Este proyecto se desarrolla con la metodología JCC. Doc: <ruta o enlace al JCC_Agentic_Dev_Methodology_vX.md>.
...
- **Operas como COPILOTO.** En las transiciones de fase, recuerda y ofrece el command que toca
  ...
  **Mantienes la documentación al día sin que te lo pidan**: sobrescribes "Fase actual" (la historia
  va a `handoffs/` + índice global, NO a esta línea), y registras cada documento nuevo en el README del cambio.
...
- **Reconciliación al arrancar.** Contrasta la "Fase actual" con los artefactos reales del repo
  (¿qué SPEC existen?, ¿qué está implementado/verificado?). Si no cuadran, **dilo**.
```
...
- **Cuándo promover plano→programa:** cuando aparece un **segundo DESIGN** en la misma carpeta (señal de que son varios cambios). No apiles `DESIGN-02/03/04` en plano: eso es un programa pidiendo subcarpetas.
...
- **Qué hace** (funde tu ritual de cierre en un acto deliberado y ordena los tres hogares):
  ...
  4. Actualiza el **README del cambio** y el **índice global** `docs/cambios/README.md`.
  5. Te **recuerda actualizar la memoria** y lo pendiente.
...
- **Los tres hogares + índice global.** Estado vivo (`CLAUDE.md`, corto, se sobrescribe) · mapa (`README` del cambio) · historia (`handoffs/`) · puerta de entrada (`docs/cambios/README.md`). CC los mantiene; nunca se apila historia en `CLAUDE.md`.
