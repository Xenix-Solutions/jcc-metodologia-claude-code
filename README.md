...
Este repositorio es la **publicación de la versión vigente**: el documento de la metodología, los cinco slash commands y su instalador. La evolución (histórico, debates de diseño, versiones archivadas) vive en un repositorio privado; aquí llega cada versión liberada.
...
- **Instalación de los commands** (Windows / PowerShell):
...
El script copia los `jcc-*.md` a `~/.claude/commands/` (ámbito de usuario: disponibles en todos tus proyectos). Es idempotente; `.\commands\install.ps1 -Check` comprueba si tu copia viva ha divergido sin escribir nada. En otros sistemas basta con copiar los `commands/jcc-*.md` a `~/.claude/commands/`.

## En una pantalla

- **Cuatro fases + cierre, cada una un slash command:**
...
- Las instrucciones de los commands atadas a un modelo concreto van etiquetadas como **calibración perecedera** (`calibración vX.Y.Z para <modelo>; revisar al cambiar de modelo`): son lo primero que se audita en cada transición de modelo.
