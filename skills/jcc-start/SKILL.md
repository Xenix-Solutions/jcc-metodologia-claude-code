...
- **INCONSISTENTE** — cualquier combinación que no encaje arriba: `jccdocs/` sin bloque JCC;
  bloque v1.5 sin `jccdocs/README.md`; bloque con versión MAYOR que la de este command (tu kit
...
- **Fase actual:** ninguno; el siguiente arranca con `/jcc-design`, `/jcc-analysis` o `/jcc-start`.
  <cuando haya trabajo activo, un puntero CORTO POR WORK ITEM ACTIVO — una línea cada uno; lo
  normal es una —: work item · fase (Design / Spec / Implementation / Review) · siguiente command
  con la barra al inicio · enlace al README del work item y a su último handoff · enlace al índice
  global `jccdocs/README.md`. Cada línea se SOBRESCRIBE en su transición y se borra al cerrar su
  work item; NUNCA acumula historia. En equipo, el puntero viaja con la rama: al mergear se
  conservan las líneas de ambas. Este texto entre <> se borra al escribir el primer puntero.>
...
- **Lectura acotada a este repo.** La sesión no lee rutas fuera de este repo (otros proyectos,
  ...
  en el mensaje: el sistema de ficheros es el mismo riesgo que los conectores.
