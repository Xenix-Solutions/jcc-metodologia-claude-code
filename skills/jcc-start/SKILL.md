...
VERSIÓN DEL CANON QUE CONOCE ESTE COMMAND: **v1.5** (la compara con la línea `Bloque JCC vX.Y`
...
`README.md`?), `git status`/`git log` (¿hay repo? ¿hay historia?). Tres casos:
...
- **DESFASADO** — hay bloque JCC pero sin línea de versión (pre-v1.5) o con versión menor que la
  ...
  si prefiero trabajar hoy sin migrar, la mezcla de versiones se tolera por diseño (la doctrina
  declarada en el CLAUDE.md del proyecto gana al texto de los skills). Sigue al PASO 3 con lo que
  hay.
- **AL DÍA** — bloque v1.5 y `jccdocs/` con su README → PASO 3.
...
`.gitignore`? Luego propón el plan y, aprobado, ejecuta:
1. `.gitignore` ANTES del primer commit: medios (`*.mp4 *.mov *.mkv *.mp3 *.wav *.m4a *.zip *.7z
   *.iso`), basura de SO/editor, secretos (`.env`, `*.local.json`) y lo propio del stack. Un
   vídeo de reunión de 100 MB en la historia de git obliga a reescribirla; ya pasó.
2. `CLAUDE.md` con el bloque JCC v1.5 de abajo (edición dirigida si el fichero ya existe: añade la
   sección, no reescribas el resto) + la sección `## Reglas operativas (INVIOLABLES)`: la línea
   de conectores MCP declarados SIEMPRE; la confidencialidad en dos niveles si es de cliente.
...
5. `git init` si no hay repo, y el primer commit ("bootstrap JCC v1.5") con mi visto bueno. Sin
   push: la política de push es del proyecto y se decide en mesa (si el repo despliega en push,
   recuérdame el checkpoint `permissions.ask` para `git push` y `gh pr create`).

PASO 3 — RECONCILIACIÓN DE APERTURA + MAPA CORTO (proyectos con estructura). Sigue los punteros
(índice global `jccdocs/README.md` → README del work item activo → su último handoff) y contrasta
la "Fase actual" con los artefactos reales y con `git log`. Dime en pocas líneas: qué work items
...
PLANTILLA DEL BLOQUE JCC v1.5 (copia instalada de la plantilla del doc de metodología; si
...
Naming: `yyyymmdd_<epic|feature|analysis>_<slug>/`; dentro de un Epic, `feature-NN_<slug>/`.

| Fecha | Tipo | Slug | Qué es | Estado | Enlaces |
|---|---|---|---|---|---|
```
